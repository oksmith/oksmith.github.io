...
- we are taking a weighted average of the rewards $r_{k}$ seen at each step. The weights are given by $\nu(a_k|x_k) / \mu(a_k|x_k)$

- the $1 / \mu(a_k|x_k)$ term acts as a debiasing factor, to account for the fact that the way the data was gathered is biased towards the live policy’s preferences

- the $\nu(a_k|x_k)$ term is in the numerator because, what we ultimately want is an _expected value_ $\mathbb{E}( . )$ under the new policy’s probabilities
